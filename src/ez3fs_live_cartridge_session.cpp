...
bool LiveCartridgeSession::open(
    std::string& error,live::Filesystem::ScanProgress verification_progress) {
    ...
    if(!filesystem_.verify(error,std::move(verification_progress))){std::string ignored;storage_.close(ignored);return false;}
