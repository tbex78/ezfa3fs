...
public:
    CountingDevice() : bytes(ezfa3fs::live::NorFlash::capacity,0xFF) {}
    ...
    bool eraseBlock(std::size_t block,std::string& error) override {
        ...
        std::fill_n(bytes.begin()+static_cast<std::ptrdiff_t>(
                        block*ezfa3fs::live::NorFlash::block_size),
                    ezfa3fs::live::NorFlash::block_size,0xFF);
        error.clear();
        return true;
    }
...
int main() {
    using ezfa3fs::live::NorFlash;
    CountingDevice device;
    device.bytes[4]=0x12;
    ezfa3fs::live::CachedBlockDevice cache(device);
    std::string error;
    std::uint8_t value=0;
